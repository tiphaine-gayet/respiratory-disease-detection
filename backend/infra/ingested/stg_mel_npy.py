"""
Snowflake internal stage for mel spectrogram .npy files from preprocessed patient audio.
One .npy per recording, named {stem}_mel.npy.
"""

import os
from ...utils.snowflake_client import SnowflakeClient

DATABASE   = os.getenv("SNOWFLAKE_DATABASE")
SCHEMA     = os.getenv("SNOWFLAKE_SCHEMA_INGESTED")
STAGE_NAME = "STG_MEL_NPY"
STAGE_FULL_PATH = f"{DATABASE}.{SCHEMA}.{STAGE_NAME}"


def create_stage(client):
    client.execute(f"CREATE STAGE IF NOT EXISTS {STAGE_FULL_PATH}")
    print(f"✅ Stage {STAGE_FULL_PATH} created (if not exists).")


if __name__ == "__main__":
    print("🚀 Setting up mel npy stage...")
    with SnowflakeClient() as client:
        create_stage(client)
    print("✅ Mel npy stage ready.")
